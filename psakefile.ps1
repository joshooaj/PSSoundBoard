...
function Update-FunctionsToExport {
    ...
    $manifestParams.FunctionsToExport = @( Get-ChildItem -Path $PSScriptRoot\PSSoundBoard\Public\*.ps1 -Recurse | Select-Object -ExpandProperty BaseName )
}
function Update-ScriptsToProcess {
    ...
    $manifestParams.ScriptsToProcess = @( Get-ChildItem -Path $PSScriptRoot\PSSoundBoard\Classes\*.ps1 -Recurse | Select-Object -ExpandProperty FullName | Resolve-Path -Relative )
    Pop-Location
}

properties {
	$script:ModuleName = 'PSSoundBoard'
	$script:CompanyName = 'Josh Hendricks'
...
Task Build {
    Get-ChildItem -Path $PSScriptRoot\output\PSSoundBoardLib\Release\PSSoundBoardLib*.* | Copy-Item -Destination $PSScriptRoot\PSSoundBoard\lib
    $srcManifestFile = Get-Item -Path $PSScriptRoot\PSSoundBoard\*.psd1 | Select-Object -First 1
    ...
    Get-ChildItem -Path $PSScriptRoot\PSSoundBoard | Copy-Item -Destination $outputDirectory.FullName -Recurse -Force
