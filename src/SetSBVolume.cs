...
namespace PSSoundBoardLib
{
  ...
  public class SBVolume : PSCmdlet
  {
    ...
    [Parameter(ParameterSetName = "Mute")] public SwitchParameter Mute { get; set; }
    ...
    {
      if (MyInvocation.BoundParameters.ContainsKey(nameof(MusicVolume)))
      {
        SoundBoard.Instance.MusicVolume = MusicVolume;
      }

      if (MyInvocation.BoundParameters.ContainsKey(nameof(EffectsVolume)))
      {
